...
class EnhancedMarkProcessor(BaseProcessor):
    """
    Procesador mejorado para la detección de marcas OMR en formularios.
    Incluye soporte para detección de diferentes tipos de marcas y
    algoritmos optimizados para mejorar la precisión.
    """
    def __init__(self):
    ...
    def preprocess_roi(self, roi, field_name: str) -> np.ndarray:
        """
        ...
        try:
            ...
            if self._debug_folder:
                ...
                cv2.imwrite(os.path.join(self._debug_folder, f"{base_filename}2_normalized.png"), normalized)
            ...
            logger.error(f"Error en preprocesamiento de ROI {field_name}: {e}", exc_info=True)
            return None
    
    def detect_shape(self, roi, field_name: str) -> Tuple[str, List[Any]]:
        """
        ...
        if field_name in self._mark_types:
        ...
        try:
            ...
            logger.error(f"Error en detección de forma para {field_name}: {e}")
            # Por defecto, asumir cuadrado
            return 'square', []
    
    def process_mark(self, roi, field_name: str) -> Tuple[bool, float, Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.error(f"Error procesando marca {field_name}: {e}", exc_info=True)
            return False, 0.0, metadata
